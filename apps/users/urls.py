from django.urls import path
from apps.users.views import UserListCreateView, BlockUserView, \
    UnBlockUserView, UserToAdminView, SendEmailTestView

urlpatterns = [
    path('', UserListCreateView.as_view(), name='users_list_create'),
    path('/<int:pk>/block', BlockUserView.as_view(), name='user_block'),
    path('/<int:pk>/unblock', UnBlockUserView.as_view(), name='user_unblock'),
    path('/<int:pk>/to_admin', UserToAdminView.as_view(), name='user_to_admin'),
    path('/test', SendEmailTestView.as_view(), name='users_send_test_email'),

]
